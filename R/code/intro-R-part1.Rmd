...
```{r}
# Set Dropbox directory (assuming you save all the data/figures/tables there)
# DBDIR <- "/Users/yixinzhou/Dropbox (BFI)/orientation_2022"

CODEDIR <- "/Users/yixinzhou/Github/"

# I like to include a script that loads all the packages and set different directories 
source(paste(CODEDIR, "directory_R.R", sep = "/"))
...
## Basic arithmatic and functions 
```{r}
...
```{r}
#import csv
dt <- paste0(DBDIR, "/data_csv/tv_hurricanes_by_network.csv") %>% 
  read.csv() %>% 
  data.table()
```

### b. UNFINISHED Data types 
...
dt[, lapply(.SD, mean), by = .(Query, yr_month), .SDcols = cols]


# create new columns that summarize the data 
new_cols <- paste0("Mean.", cols) # new column names 
dt[, (new_cols) := lapply(.SD, mean), by = .(Query, yr_month), .SDcols = cols]
head(dt)
...
** In Stata, since you specify 1:1 or 1:m or m:1 when writing code, mis
...
assert(dim(with_location)[1] == dim(dt)[1]) # make sure all hurricanes in dt are matched 
assert(sum(with_location$Query == "Hurrican Ida") == 0) # "Hurrican Ida" should not be in the dataset 
